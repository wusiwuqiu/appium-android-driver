 ...
 */
commands.aAdbExec = async function aAdbExec (cmd, opts = {}) {
  return await this.adb.adbExec(cmd, opts);
};
 ...
 * @throws {Error} If the command returned non-zero exit code.
 */
commands.aAdbShell = async function aAdbShell (cmd, opts = {}) {
  return await this.adb.shell(cmd, opts);
};

commands.reapAction = async function reapAction (actions) {
  for (let g of actions) {
